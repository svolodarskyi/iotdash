...
def test_list_devices_unauthenticated(client):
    response = client.get("/api/devices")
    assert response.status_code == 401


def test_list_devices_with_data(auth_client, seed_data):
...
def test_get_device(auth_client, seed_data):
...
def test_get_device_not_found(auth_client):
    ...
    response = auth_client.get(f"/api/devices/{fake_id}")
    assert response.status_code == 404
    assert response.json()["detail"] == "Device not found"


def test_get_device_embed_urls(auth_client, seed_data):
    device_id = str(seed_data["devices"][0].id)
    response = auth_client.get(f"/api/devices/{device_id}/embed-urls")
    assert response.status_code == 200
    data = response.json()
    assert data["device_code"] == "test_sensor01"
...
def test_get_device_embed_urls_not_found(auth_client):
    fake_id = str(uuid.uuid4())
    response = auth_client.get(f"/api/devices/{fake_id}/embed-urls")
    assert response.status_code == 404
