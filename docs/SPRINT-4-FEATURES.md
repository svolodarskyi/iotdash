...
- **Terraform module** (`infra/soak-test/`) provisions complete Azure environment:
  ...
  - VNet + single subnet
  - NSG with rules for SSH (restricted to caller IP), MQTT (1883), backend (8000), frontend (5173), Grafana (3000), EMQX Dashboard (18083)
...
- **Frontend** (`frontend/Dockerfile.prod`): multi-stage build, Node 20 Alpine → nginx Alpine, gzip compression, static asset caching (1y immutable), SPA routing via try_files
- Both images pushed to GHCR as `ghcr.io/<owner>/iotdash-backend` and `ghcr.io/<owner>/iotdash-frontend`
...
- **Frontend VITE_API_URL baked at build time** — needs rebuild for different VM IPs (could use runtime config injection)
