...
| `vm.tf` | VM, NIC, NSG (6 rules), public IP, cloud-init |
| `variables.tf` | 13 configurable inputs |
| `outputs.tf` | VM IP, SSH command, service URLs, ACR credentials |
...
| 4 | Network Security Group | `nsg-iotdash-soak` | 6 inbound rules (see below) |
| 5 | Public IP | `pip-iotdash-soak` | Dynamic, Basic SKU → `13.82.138.72` |
...
```bash
ssh -i ~/.ssh/iotdash-soak azureuser@13.82.138.72
```
...
| Service | URL |
|---------|-----|
| Backend API | http://13.82.138.72:8000 |
| Frontend | http://13.82.138.72:5173 |
| Grafana | http://13.82.138.72:3000 |
| EMQX Dashboard | http://13.82.138.72:18083 |
| InfluxDB | http://13.82.138.72:8086 |
...
```bash
ssh -i ~/.ssh/iotdash-soak azureuser@13.82.138.72 'sudo tail -f /var/log/cloud-init-output.log'
```
...
```bash
ssh -i ~/.ssh/iotdash-soak azureuser@13.82.138.72 'cloud-init status --wait'
```

### 2. Verify Stack is Running

```bash
ssh -i ~/.ssh/iotdash-soak azureuser@13.82.138.72 'docker compose -f /opt/iotdash/docker-compose.yml -f /opt/iotdash/docker-compose.soak.yml ps'
```
