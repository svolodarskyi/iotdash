...
- [ ] Image runs:
  ```bash
  docker run --rm -p 5173:5173 iotdash-frontend:test
  ```
- [ ] `curl -I localhost:5173` returns `200 OK` with nginx headers
- [ ] `curl localhost:5173` returns HTML (index.html)
- [ ] `curl localhost:5173/nonexistent-route` returns index.html (SPA fallback working)
- [ ] `curl -I localhost:5173/assets/` — check for cache headers (`Cache-Control: public, immutable`)
- [ ] `curl localhost:5173/api/health` returns `{"status":"ok"}` (nginx reverse proxy to backend)
- [ ] Stop container, clean up
...
- [ ] Trigger: `workflow_dispatch` with inputs + auto on push to main (for relevant paths)
- [ ] Job 1 (`build-and-push`):
  - [ ] Logs into ACR (`criotdashsoak.azurecr.io`)
  - [ ] Builds backend with `Dockerfile.prod`
  - [ ] Builds frontend with `Dockerfile.prod`
  - [ ] Pushes both with `latest` + SHA tags to ACR
...
- [ ] `SOAK_VM_HOST` — VM public IP or DNS hostname
- [ ] `SOAK_VM_SSH_KEY` — private SSH key for VM access
- [ ] `ACR_LOGIN_SERVER` — ACR login server URL (`criotdashsoak.azurecr.io`)
...
# - Port conflicts (soak adds 80:5173, base has 5173:5173 — both should work via port merge)
