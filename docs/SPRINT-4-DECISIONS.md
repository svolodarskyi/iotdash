...
## D3: GHCR Over Azure Container Registry

**Decision:** Push production Docker images to GitHub Container Registry (ghcr.io) instead of ACR.

**Why:** The repo is public, so GHCR is free with no auth needed for pulls. ACR would cost ~EUR 5/month (Basic SKU) and require additional auth configuration. Since the soak test VM just needs to pull images, GHCR with public packages is the simplest path.

**Production:** Revisit for production — ACR has better Azure integration and private image support.
