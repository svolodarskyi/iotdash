...
│  │  Inbound rules (all restricted to your IP):              │   │
│  │  22, 1883, 3000, 5173, 8000, 18083                       │   │
│  └──────────────────────────────────────────────────────────┘   │
│                                                                 │
│  ┌──────────────────────────────────────────────────────────┐   │
...
|------|---------|---------------|
| 22 | SSH | Remote access to the VM for debugging and management |
| 1883 | MQTT (EMQX) | So you can connect test MQTT clients from your laptop |
| 3000 | Grafana | View dashboards and monitoring data in your browser |
| 5173 | Frontend (nginx) | Access the IoTDash web application |
| 8000 | Backend (FastAPI) | API access for testing and the frontend |
| 18083 | EMQX Dashboard | Monitor MQTT connections, message rates, client status |

**Security note:** All rules use `source_address_prefix = your_ip/32`. If your IP changes (e.g. you reconnect to a different network), you'll need to update the NSG rule or the terraform variable `allowed_ssh_cidr` and re-apply.
...
**What it is:** A public IPv4 address (`13.82.138.72`) attached to the VM so you can reach it from the internet.

**Why Dynamic + Basic SKU:** Dynamic IPs are free when allocated to a running VM. Basic SKU is the cheapest option. The IP may change if the VM is deallocated and restarted (not rebooted — full stop/start). For a temporary soak test, this is fine.

**What it enables:** SSH access, browser access to Grafana/frontend/backend, MQTT client connections from your laptop.

**Analogy:** The building's street address.
...
- `VITE_API_URL` and `VITE_GRAFANA_URL` are baked in at build time via build args
...
│ 3. Build backend    │     │ 3. docker pull images │
│ 4. Build frontend   │     │ 4. docker compose up  │
