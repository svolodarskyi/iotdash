...
```
1. GitHub Actions:
   - soak-deploy.yml: build images → push to ACR → SSH deploy to VM (auto-triggers on push to main)
...
### Sprint 12 — Grafana Auth Integration (Backlog)

**Goal:** Grafana dashboards are only visible to authenticated users. Anonymous access is disabled. Embedded iframes authenticate via service account tokens. Admins can still access Grafana directly with credentials.

**Problem:** Grafana is currently exposed on port 3000 with anonymous access. Any user who knows the IP can view dashboards without logging in. Disabling anonymous access breaks the embedded iframes in the frontend (they show Grafana's login page instead of the dashboard).

**Tasks:**

```
1. Grafana service account tokens:
   - Create a Grafana service account per org (or one shared viewer account)
   - Generate API tokens for iframe authentication
   - Backend stores tokens, injects into embed URLs as auth headers

2. Proxy Grafana through nginx:
   - Add /grafana location in frontend nginx config
   - Proxy to grafana:3000 with auth token injection
   - Remove direct port 3000 exposure from docker-compose
   - Update NSG: remove Grafana port 3000 rule

3. Frontend embed URL rewriting:
   - Update rewriteGrafanaUrl() to use /grafana proxy path
   - Iframes load through same-origin nginx proxy (no CORS issues)
   - Auth token injected server-side by nginx (not exposed to browser)

4. Admin direct access:
   - Admins access Grafana via http://<host>/grafana with Grafana login
   - Or via SSH tunnel: ssh -L 3000:localhost:3000 azureuser@<host>
   - Default credentials: admin / admin (change on first login)

5. Per-org dashboard isolation:
   - Grafana orgs map to IoTDash orgs
   - Service account tokens scoped to specific Grafana org
   - Users can only see dashboards for their org's devices
```

**Sprint Review Deliverable:** Anonymous user visiting the app or Grafana URL sees a login page. Authenticated app users see embedded dashboards without extra login. Admins access Grafana directly with credentials. No IoT data visible without authentication.
...
| Sprint 12 | TBD  | Grafana auth integration, proxy, per-org isolation | No anonymous dashboard access   |
